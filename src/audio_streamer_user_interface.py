...
class StreamerUserInterface(UserInterface):
    ...
    async def get_view(self) -> View:
        ...
        if self.queue.get_current_audio() is not None:
            pause_button = Button(style=ButtonStyle.secondary, emoji='▶' if self.voice.is_paused() else '⏸')
        else:
