...
from async_event_handler import EventBus
...
class AudioStreamer(commands.Cog):
    ...
    @slash_command(name='play', description='Queue YouTube audio files via a search query or URL', guild_ids=GUILD_IDS)
    ...
    async def play_command(self, ctx: ApplicationContext, query: str) -> None:
        ...
        audio_title = await self.queue_audio(session=session,
                                             author=ctx.author,
    ...
    @slash_command(name='play_next', description='Queue up next YouTube audio files via a search query or URL', guild_ids=GUILD_IDS)
    @option('query', description='Search or URL', required=True)
    ...
    @slash_command(name='playlist', description='Queue a series of audio files from a YouTube Playlist URL', guild_ids=GUILD_IDS)
    ...
    async def playlist_command(self, ctx: ApplicationContext, url: str) -> None:
        ...
        get_running_loop().create_task(coro=self.queue_playlist(session=session,
                                                                author=ctx.author,
                                                                voice_channel=ctx.author.voice.channel,
                                                                text_channel=ctx.channel,
                                                                urls=playlist['urls']),
    ...
    @slash_command(name='pause', description='Pause the currently playing audio', guild_ids=GUILD_IDS)
    async def pause_command(self, ctx: ApplicationContext) -> None:
        ...
        if session.voice.pause_playback():
            ...
            await ctx.respond(embed=Embed(title='Paused', description=f'**{session.voice.cur_audio}**', color=green),
                              delete_after=DELETE_TIMER)
        else:
    ...
    async def resume_command(self, ctx: ApplicationContext) -> None:
        ...
        if session.voice.resume_playback():
            await session.user_interface.refresh_ui()
            await ctx.respond(embed=Embed(title='Resumed', description=f'**{session.voice.cur_audio}**', color=green),
                              delete_after=DELETE_TIMER)
        else:
            await ctx.respond(embed=Embed(title='Error', description='There is currently no paused audio', color=red),
                              delete_after=DELETE_TIMER)

    @slash_command(name='remove_at', description='Remove the song at an up next queue position', guild_ids=GUILD_IDS)
    @option('position', input_type=int, min_value=1, description='Queue position as shown in the Next list')
    async def remove_at_command(self, ctx: ApplicationContext, position: int) -> None:
        ...
        if audio_title:
            await ctx.respond(embed=Embed(title='Removed from Queue', description=f'Removed **{audio_title}** (position {position})', color=green),
                              delete_after=DELETE_TIMER)
        else:
            await ctx.respond(embed=Embed(title='Error', description=f'No song at queue position **{position}**', color=red),
                              delete_after=DELETE_TIMER)

    @slash_command(name='skip_to', description='Skip ahead to the song at an up next queue position', guild_ids=GUILD_IDS)
    @option('position', input_type=int, min_value=1, description='Queue position as shown in the Next list')
    async def skip_to_command(self, ctx: ApplicationContext, position: int) -> None:
        ...
        if audio_title:
            await ctx.respond(embed=Embed(title='Skipping To', description=f'**{audio_title}**', color=green),
                              delete_after=DELETE_TIMER)
        else:
            await ctx.respond(embed=Embed(title='Error', description=f'No song at queue position **{position}**', color=red),
                              delete_after=DELETE_TIMER)

    @slash_command(name='restart_queue', description='Start playing from the beginning of the previous queue', guild_ids=GUILD_IDS)
    ...
    @slash_command(name='go_to', description='Go to a specific time in the audio. If no time is set, the audio is reset to the start', guild_ids=GUILD_IDS)
    ...
    @slash_command(name='clear_previous_queue', description='Clear the previous queue', guild_ids=GUILD_IDS)
    ...
    @slash_command(name='remove', description='Skips and removes the currently playing song from the queue', guild_ids=GUILD_IDS)
    ...
    @slash_command(name='reset', description='Reset the voice client, queue, and user interface', guild_ids=GUILD_IDS)
    async def reset_command(self, ctx: ApplicationContext) -> None:
        ...
        await session.user_interface.delete_ui(bot=self.bot, guild=ctx.guild, ignore_msg_ids={ctx_message_id})
    ...
    @slash_command(name='reconnect_bot', description='Reconnect the bot to your voice channel and text channel', guild_ids=GUILD_IDS)
    ...
    async def on_ready(self):
        ...
        logging.info('AudioStreamer ready in guilds: %s', [guild.id for guild in self.bot.guilds if guild.id in GUILD_IDS])
        ...
        if entry:
            ...
            try:
                if not session.voice.is_connected():
                    logging.info('Voice not connected, attempting to join voice channel for queued audio')
    ...
    async def queue_playlist(self,
                             ...
                             urls: list[str]) -> None:
        for url in urls:
            await self.queue_audio(session=session,
                                   author=author,
                                   voice_channel=voice_channel,
                                   text_channel=text_channel,
                                   query=url)
